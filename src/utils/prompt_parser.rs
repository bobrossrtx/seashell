...
impl Prompt for SeashellPrompt {
    fn render_prompt_left(&self) -> std::borrow::Cow<str> {
        let prompt_template = env_vars::get_var_as_string("PROMPT").unwrap_or_else(|_| "[{USER}@{HOSTNAME}]-[{PWD}] $".to_string());
...
pub fn parse_prompt(template: &str) -> String {
    ...
    while let Some(c) = chars.next() {
        if c == '/' {
            ...
            if let Ok(value) = env_vars::get_var_as_string(&var_name) {
                let mut styled_value = value;
                for style in &current_styles {
                    match style.as_str() {
                        "bold" => styled_value = styled_value.bold().to_string(),
                        "underline" => styled_value = styled_value.underline().to_string(),
                        color => styled_value = styled_value.color(color).to_string(),
                    }
                }
                result.push_str(&styled_value);
                current_styles.clear(); // Reset styles after applying to the variable
            } else {
        ...
        current_styles.clear(); // Reset styles after applying to the character
    }
